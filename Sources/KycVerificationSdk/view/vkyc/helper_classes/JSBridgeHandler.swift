...
class JSBridgeHandler: NSObject, WKScriptMessageHandler {
    ...
    private func handleVKycOnClose(_ data: String?) {
           print("handleVKycOnClose triggered with data: \(String(describing: data))")
           
           guard let data = data, let jsonData = data.data(using: .utf8),
                 let json = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any],
                 let verificationId = json["verificationId"] as? String else {
               print("Failed to parse data or verificationId is missing")
               return
           }
           ...
           DispatchQueue.main.async { [weak self] in
               guard let self = self, let viewController = self.viewController else {
                   print("Self or viewController is nil, cannot proceed.")
                   return
               }
               
               if viewController.isViewLoaded && viewController.view.window != nil {
                   // ViewController is in the hierarchy
                   if let navigationController = viewController.navigationController {
                       navigationController.popViewController(animated: true)
                   } else if viewController.presentingViewController != nil {
                       viewController.dismiss(animated: true, completion: nil)
                   } else {
                       print("ViewController is not presented modally and no navigation controller found.")
                   }
               } else {
                   print("ViewController is not in the view hierarchy.")
               }
           }
       }
